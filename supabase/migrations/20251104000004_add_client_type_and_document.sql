 ...
 * - document: CNPJ ou CPF com máscara aplicada
 * 
...
ALTER TABLE public.workspaces
  ...
  ADD COLUMN document VARCHAR(18);
 ...
 * 
 * Valida o comprimento do documento baseado no tipo de cliente:
 * - Pessoa Física (CPF): 14 caracteres com máscara (###.###.###-##)
 * - Pessoa Jurídica (CNPJ): 18 caracteres com máscara (##.###.###/####-##)
 * 
 * Nota: A validação de dígitos verificadores é feita no frontend e backend,
 *       esta constraint apenas garante o formato da string.
 */
ALTER TABLE public.workspaces
  ADD CONSTRAINT check_document_format CHECK (
    CASE
      WHEN client_type = 'pessoa_fisica' THEN LENGTH(document) = 14  -- CPF: ###.###.###-##
      WHEN client_type = 'pessoa_juridica' THEN LENGTH(document) = 18  -- CNPJ: ##.###.###/####-##
    END
  );
...
'Documento do cliente (CNPJ ou CPF) com máscara. Formato: CPF ###.###.###-## ou CNPJ ##.###.###/####-##. Campo único para impedir duplicação de clientes.';
